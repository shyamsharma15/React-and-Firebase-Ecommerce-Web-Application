...
function Cart() {
  ...
  useEffect(() => { // jisse jab cart mai se product delete karein toh local storage se bhi wo delete ho
  ...
  const buyNow = async () => {
    ...
    var options = {
      key: "rzp_test_146Rcsr5Tu1su0",
      key_secret: "VJy97PYc2DW0UenkI1l3cb",
      ...
      handler: function (response) {
        ...
        const orderInfo = {
          ...
          email: JSON.parse(localStorage.getItem("user")).user.email, // parse isliye liya taaki object ki form mai data aaye local storage mai se
          userid: JSON.parse(localStorage.getItem("user")).user.uid,
          paymentId

        }

        try { // firestore mai store karinge 
          const orderRef = collection(fireDB, 'order');
          addDoc(orderRef, orderInfo) // addDoc do cheez leta hai ekk to data reference leta hai aur doosera yea ki konsa data store karna hai  
  ...
  return (
    <Layout>
      {/* Cart Page Wrapper */}
      <div
        className="min-h-screen flex flex-col bg-gray-100 pt-5"
        style={{
      ...
      >
        ...
        <div className="flex-grow mx-auto max-w-5xl px-6 md:flex md:space-x-6 xl:px-0">
          {/* Cart Items Section */}
          <div className="rounded-lg md:w-2/3 overflow-y-auto max-h-[calc(100vh-150px)]">
          ...
          <div
            className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/2"
            style={{
              backgroundColor: mode === "dark" ? "rgb(32 33 34)" : "",
              color: mode === "dark" ? "white" : "",
            }}
